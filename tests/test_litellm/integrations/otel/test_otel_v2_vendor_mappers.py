...
def test_openinference_mapper_input_output_messages():
    attrs = OpenInferenceMapper().map(SpanRole.LLM_CALL, _llm_call())
...
def test_openinference_mapper_skips_non_llm_roles():
    ...
    assert (
        OpenInferenceMapper().map(SpanRole.GUARDRAIL, GuardrailSpanData("presidio"))
        == {}
    )
...
def test_openinference_multimodal_content_text_only():
    ...
    attrs = OpenInferenceMapper().map(SpanRole.LLM_CALL, data)
...
def test_langfuse_mapper_observation_attrs():
    attrs = LangfuseMapper().map(SpanRole.LLM_CALL, _llm_call())
...
def test_langfuse_mapper_skips_when_no_messages():
    ...
    attrs = LangfuseMapper().map(SpanRole.LLM_CALL, data)
...
def test_weave_mapper_display_and_output():
    attrs = WeaveMapper().map(SpanRole.LLM_CALL, _llm_call())
...
def test_langtrace_mapper_attrs():
    attrs = LangtraceMapper().map(SpanRole.LLM_CALL, _llm_call())
...
def test_resolve_mappers_composition_layers_vocabularies():
    ...
    union: dict = {}
    for mapper in chain:
        union.update(mapper.map(SpanRole.LLM_CALL, data))
    # Canonical
