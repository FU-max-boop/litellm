...
def json_if(payload: Mapping[str, object]) -> Optional[str]:
    """JSON-serialize ``payload`` only when it's non-empty; else ``None``."""
    return json.dumps(payload) if payload else None


def json_or_none(value: object) -> Optional[str]:
...
def stringify_message(message: object) -> Optional[str]:
...
def serialize_messages(messages: Sequence[object]) -> Optional[str]:
...
def message_content(message: object) -> Optional[str]:
